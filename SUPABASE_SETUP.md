...
## Шаг 4: Включение Realtime (КРИТИЧЕСКИ ВАЖНО!)

**БЕЗ ЭТОГО ШАГА ИГРА НЕ БУДЕТ РАБОТАТЬ В РЕАЛЬНОМ ВРЕМЕНИ!**

1. В Supabase Dashboard перейдите в **Database** → **Replication**
2. Найдите раздел "Tables" или "Publications"
3. **Включите Realtime для следующих таблиц:**
   - ✅ `players`
   - ✅ `games`
   - ✅ `guesses`
4. Нажмите кнопку "Enable" или переключите тумблер для каждой таблицы
5. Подождите несколько секунд пока изменения применятся

**Альтернативный способ:**
- Settings → API → убедитесь что Realtime enabled

После включения Realtime:
- Обновления игры будут приходить мгновенно
- Ходы оппонента будут видны сразу
- История попыток синхронизируется автоматически
...
5. Проверьте, что:
   ...
   - В консоли видны логи: "Game update received", "New guess received"
